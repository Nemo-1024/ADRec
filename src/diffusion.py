...
class DenoisedModel(nn.Module):
    def __init__(self, args):
        ...
        self.transencoder = TransformerEncoder(args,num_blocks=4,norm_first=False,hidden_size=self.hidden_size)
    ...
    def forward(self, rep_item, x_t, t, mask_seq,mask_tgt,condition=True):
        ...
        rep_diffu = (rep_item + lambda_uncertainty * (x_t + time_emb))*mask_tgt.unsqueeze(-1)
...
class AdRec(nn.Module):
    ...
    def subseq_guidence(self,seq, tgt, mask_seq, mask_tag):
        b,l,d = seq.shape
        rel_dist=l - torch.arange(1,l+1,device=seq.device)
        k = 4
        tgt = tgt.repeat(k, 1, 1)
        mask=torch.rand([b*k,l],device=tgt.device) < (rel_dist.unsqueeze(0)/l)
        noise_x_t = th.randn_like(tgt)
        tgt=torch.where((mask==1).unsqueeze(-1), noise_x_t, tgt)
        seq = seq.repeat(k, 1, 1)
        ...
        for i in indices:  # from T to 0, reversion iteration
            ...
            noise_x_t = torch.concat([tgt[:, 1:], noise_x_t[:, -1:]], dim=1)
            # noise_x_t = torch.concat([torch.zeros_like(tgt[:, 1:]),noise_x_t[:, -1:]], dim=1)
            noise_x_t = self.p_sample(seq, noise_x_t, t, mask_seq.repeat(k,1), mask_tag.repeat(k,1))
        noise_x_t = noise_x_t.reshape(k,b,l,d).mean(0)
        return noise_x_t
